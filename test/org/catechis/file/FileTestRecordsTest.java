...
* use the loaded doc, so we used this method to create sanitized versions...
    ...
    */
    ...
    public void testUpdateTestsStatus()
    {
	    ...
	    try
	    {
	    	original = ftr.getTestsStatus(user_name, subject);
	    	actual_size = ftr.updateTestsStatus(user_name, subject);
	    } catch (java.lang.NullPointerException npe)
	    {
	    	System.out.println("FileTestRecordsTest.testUpdateTestsStatus NPE");
		    dumpLog(ftr.getLog());
	    }
    ...
    */
    ...
    /**
     * ftr.getTestsStatus(user_id, subject);
     */
    public void testGetTestsStatus()
    {
	    ...
	    FileTestRecords ftr = new FileTestRecords(root_path);
	    int grand_index = 0;
    	int daily_session_tests = 0;
    	try
    	{
    		grand_index = ftr.getTestsStatus(user_id, subject);	// this gets the the grand total of tests
    		daily_session_tests = ftr.getDailySessionTests();
    		//System.out.println("daily_session_tests "+daily_session_tests);
    	} catch (java.lang.NullPointerException npe)
    	{
    	...
    	int actual = daily_session_tests;
	    int expected = 1;
	    //System.out.println("FileTestRecordsTest.testGetTestsStatus actual "+actual+" gi "+grand_index+" ---- log ---");
	    //dumpLog(ftr.getLog());
	    assertEquals(expected, actual);
    }
    
    /**
     * updateTestsStatus(user_id, subject);
     */
    public void testResetDailySessionTestsStatus()
    {
    	String user_id = new String("test_user");
	    String subject = new String("vocab");
	    File path_file = new File("");
	    String root_path = path_file.getAbsolutePath();
	    FileTestRecords ftr = new FileTestRecords(root_path);
	    ftr.getTestsStatus(user_id, subject);	// this gets the the grand total of tests
		int original = ftr.getDailySessionTests();
    	int grand_index = ftr.updateTestsStatus(user_id, subject);
    	ftr.resetDailySessionTestsStatus(user_id, subject);
    	int daily_session_tests = ftr.getDailySessionTests();
    	int actual = daily_session_tests;
     ...
     * updateTestsStatus(user_id, subject);
     */
    public void testUpdateDailySessionTestsStatus()
    {
    	String user_id = new String("test_user");
	    String subject = new String("vocab");
	    File path_file = new File("");
	    ...
	    long start = Long.parseLong("1318474138743");
	    long end = Long.parseLong("1318474138712");
	    ...
	    System.out.println("FileTestRecordsTest.testGetTestsStatus original "+original+" actual "+actual+" ---- log ---");
	    dumpLog(ftr.getLog());
	    assertEquals(expected, actual);
    }
    
    /**
     * get/setDailySessionStartTime(String user_name, String subject, long time)
     */
    public void testSetAndGetDailySessionStartTime()
    {
    	String user_id = new String("test_user");
	    String subject = new String("vocab");
	    File path_file = new File("");
    	...
    	ftr.setDailySessionStartTime(user_id, subject, time);
    	long actual = ftr.getDailySessionStartTime(user_id, subject);
	    long expected = time;
	    //System.out.println("FileTestRecordsTest.testGetTestsStatus actual "+actual+" gi "+grand_index+" ---- log ---");
	    //dumpLog(ftr.getLog());
	    assertEquals(expected, actual);
     ...
     * get/setDailySessionStartTime(String user_name, String subject, long time)
     */
    public void testGetSessionsReport()
    {
    	String user_id = new String("test_user");
	    String subject = new String("vocab");
	    File path_file = new File("");
	    String root_path = path_file.getAbsolutePath();
	    FileTestRecords ftr = new FileTestRecords(root_path);
	    Momento m = ftr.getStatusRecord(user_id, subject);
	    SessionsReport sr = ftr.getSessionsReport();
    	Date date = new Date();
    	long time = date.getTime();
    	ftr.updateSessionStatus();
    	ftr.setDailySessionStartTimeUsingSharedDoc(time);
    	ftr.setTestStartTime(); // called in IntegratedTestAction
    	long daily_session_start_time = ftr.getDailySessionStartTime();
    	int daily_session_tests = ftr.getDailySessionTests();
    	Hashtable reading_levels = ftr.getReadingLevels();
    	int grand_index = ftr.getGrandIndex();
    	String previous_level = "0";
    	String new_level = "1";
    	ftr.updateWordLevels(previous_level, new_level, Constants.READING);
    	Hashtable new_reading_levels = ftr.getReadingLevels();
    	
    	long actual = ftr.getDailySessionStartTimeUsingSharedDoc();
    	ftr.updateWordLevels(new_level, previous_level, Constants.READING);
    	ftr.writeDocument();
	    long expected = time;
	    System.out.println("ftr log --=-==----------=--");
	    dumpLog(ftr.getLog());
	    System.out.println("daily_session_start_time "+daily_session_start_time+" = "+time);
	    System.out.println("daily_session_tests "+daily_session_tests);
	    System.out.println("grand_index "+grand_index);
	    System.out.println("okd reading levels ");
	    dumpLog(reading_levels);
	    System.out.println("new reading levels ");
	    dumpLog(new_reading_levels);
	    assertEquals(expected, actual);
    }
     ...
     * get/setDailySessionStartTime(String user_name, String subject, long time)
     */
    public void testGetNumberOfRetiredWords()
    {
    	String user_id = new String("test_user");
	    String subject = new String("vocab");
	    File path_file = new File("");
     ...
     */
    public void testIncrementRetiredWords()
    {
    	System.out.println("FileStorageTest.testIncrementRetiredWords ------------------");
    	File path_file = new File("");
    	String user_id = new String("-5519451928541341468");
	    String current_dir = path_file.getAbsolutePath();
	    // first get the number of retired words and increment it to test against.
		FileTestRecords ftr = new FileTestRecords(current_dir);
		ftr.getStatusRecord(user_id, Constants.VOCAB); // this returns a momento object that we don't need here
		System.out.println("FileStorageTest.testIncrementRetiredWords ----- last_record");
		Hashtable last_record = ftr.getReadingAndWritingLevels();
		int expected_number_of_retired_words = Integer.parseInt((String)last_record.get("number_of_retired_words"));
		System.out.println("FileStorageTest.testIncrementRetiredWords ----- before increment: "+expected_number_of_retired_words);
		expected_number_of_retired_words++;
		System.out.println("FileStorageTest.testIncrementRetiredWords ----- after  increment: "+expected_number_of_retired_words);
		//ftr.resetLog();
		ftr = new FileTestRecords(current_dir);
		ftr.incrementRetiredWords(user_id, Constants.VOCAB);
		// now load the number again and see if it's changed.
		ftr.getStatusRecord(user_id, Constants.VOCAB); // this reloads all the values
		last_record = ftr.getReadingAndWritingLevels();
		int actual_number_of_retired_words = Integer.parseInt((String)last_record.get("number_of_retired_words"));
		System.out.println("FileStorageTest.testIncrementRetiredWords ----- from last_record: "+actual_number_of_retired_words);
		System.out.println("FileStorageTest.testUpdateHistory ----- last_record");
		dumpLog(last_record);
		System.out.println("FileStorageTest.testUpdateHistory ----- log");
	    dumpLog(ftr.getLog());
